...
class OrganizerRegisterSerializer(serializers.Serializer):
    ...
    # Not uniqueness-checked: these are EncryptedCharFields on
    # OrganizerProfile, and per that model's own docstring, encrypted
    # ciphertext is non-deterministic, so uniqueness lookups against them
    # are not reliable (and User has no such fields to check against
    # anyway). Format validation still applies.
    citizenship_number = serializers.CharField(
        max_length=150,
        validators=[validate_citizenship_number],
    )
