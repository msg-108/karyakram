...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null);
    try {
      // Backend LoginView (SimpleJWT) uses `username` + `password`
      const { data } = await api.post("/auth/login/", {
        username: form.username,
        password: form.password,
      });
      // data contains: { access, refresh } — user info is in the JWT claims
      ...
      // DRF returns errors as { detail: "..." } or { username: [...], password: [...] }
      const msg =
        data?.detail ||
          ...
          <div className="relative flex flex-col items-center text-center gap-8">
            ...
            <div>
              <h2 className="text-white font-bold text-2xl leading-snug">
                Discover &amp; Book<br />Events in Nepal
              </h2>
              <p className="text-white/70 text-sm mt-3 leading-relaxed max-w-xs mx-auto">
                From concerts to conferences — find, manage, and attend events that matter to you.
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 flex items-center justify-center px-6 py-12">
          <div className="w-full max-w-sm">
            <div className="mb-8">
              ...
              <p className="text-sm text-[var(--color-muted)] mt-1">Sign in to your Karyakram account</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              {/* Username — backend LoginView uses username field */}
              <div>
                ...
                <input
                  ...
                  className="w-full border border-[var(--color-border)] bg-white rounded-lg px-4 py-2.5 text-sm text-[var(--color-text)] placeholder:text-[var(--color-muted)] outline-none focus:ring-2 focus:ring-[var(--color-primary-500)] focus:border-transparent transition"
                />
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <button type="button" className="text-xs text-[var(--color-primary-500)] hover:underline">
                    Forgot password?
                  </button>
                </div>
                <div className="relative">
                  <input
                    ...
                    className="w-full border border-[var(--color-border)] bg-white rounded-lg px-4 py-2.5 pr-10 text-sm text-[var(--color-text)] placeholder:text-[var(--color-muted)] outline-none focus:ring-2 focus:ring-[var(--color-primary-500)] focus:border-transparent transition"
                  />
              ...
              <div className="flex items-center gap-2.5">
                <input id="remember" type="checkbox" checked={form.remember} onChange={set("remember")}
                  className="size-4 rounded border-[var(--color-border)] accent-[var(--color-primary-500)] cursor-pointer" />
            ...
            <p className="text-center text-sm text-[var(--color-muted)] mt-6">
              ...
              <a href="/register" className="text-[var(--color-primary-500)] font-medium hover:underline">
                Create account
              </a>
            </p>
          </div>
